...
namespace Narcissus
{
    ...
    class WndProcHook
    {
        ...
        public delegate void OnMapfileEventHandler(int length);   //热键事件委托
        public event OnMapfileEventHandler OnMapfile = null;      //热键事件
        ...
        public WndProcHook(Window win, WndProcHook.KeyFlags control, Keys key)
        {
            ...
            if (WndProcHook.KeyPair.ContainsKey(KeyId))
            {
                throw new Exception("热键已经被注册!");
            }
            //注册热键
            if (false== WndProcHook.RegisterHotKey(Handle, KeyId, ControlKey, Key))
            {
                throw new Exception("热键注册失败!");
            }
            //消息挂钩只能连接一次!!
            if(WndProcHook.KeyPair.Count == 0)
            {
                if (false == InstallHotKeyHook(this))
                    throw new Exception("消息挂钩连接失败!");
            }
            //添加这个热键索引
            WndProcHook.KeyPair.Add(KeyId, this);
        }
        ...
        //安装热键处理挂钩
        static private bool InstallHotKeyHook(WndProcHook hk)
        {
            ...
            //挂接事件
            source.AddHook(WndProcHook.HotKeyHook);
            return true;
        }
        ...
        static private IntPtr HotKeyHook(IntPtr hwnd, int msg, IntPtr wParam, IntPtr lParam, ref bool handled)
        {
            ...
            {
                case WM_HOTKEY:
                    hk = (WndProcHook)WndProcHook.KeyPair[(int)wParam];
                    if (hk.OnHotKey != null) hk.OnHotKey();
                    break;
                case 0x0401:
                    hk = (WndProcHook)WndProcHook.KeyPair[0x1 + 32 * 10];
                    if (hk.OnMapfile != null) hk.OnMapfile((int)wParam);
                    break;
            }
            return IntPtr.Zero;
        }
